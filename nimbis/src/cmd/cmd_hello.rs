...
pub(super) async fn execute(_storage: &Storage, args: &[Bytes], ctx: &CmdContext) -> RespValue {
	...
	if proto == 2 {
		resp2_hello(proto, ctx.client_id)
	} else {
		resp3_hello(proto, ctx.client_id)
	}
}
...
mod tests {
	...
	use super::resp2_hello;
	use super::resp3_hello;

	#[test]
	fn test_parse_proto_default_to_resp2() {
		let proto = parse_proto(&[]).expect("parse proto should succeed");
		assert_eq!(proto, 2);
	}

	#[test]
	fn test_parse_proto_resp2() {
		let proto = parse_proto(&[Bytes::from_static(b"2")]).expect("parse proto should succeed");
		assert_eq!(proto, 2);
	}

	#[test]
	fn test_parse_proto_resp3() {
		let proto = parse_proto(&[Bytes::from_static(b"3")]).expect("parse proto should succeed");
		assert_eq!(proto, 3);
	}

	#[test]
	fn test_parse_proto_rejects_invalid_version() {
		let err = parse_proto(&[Bytes::from_static(b"4")]).expect_err("should error");
		assert_eq!(
			err,
	...
	fn test_parse_proto_rejects_too_many_arguments() {
		let err = parse_proto(&[Bytes::from_static(b"3"), Bytes::from_static(b"SETNAME")])
	...
	fn test_parse_proto_rejects_non_utf8_input() {
		let err = parse_proto(&[Bytes::from_static(&[0xFF])]).expect_err("should error");
		assert_eq!(
			err,
			RespValue::error("NOPROTO unsupported protocol version. Use 2 or 3")
		);
